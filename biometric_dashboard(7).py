...
if not data.empty:
    ...
    # Department Comparison has its own department selector.
    # Keep the sidebar Year and Age filters, but do not apply the sidebar
    # Department filter to this tab so "All Departments" truly means all.
...
with tab_risk:
    ...
    if data.empty:
        ...
        # The Risk Factors tab has its own department and year filters so it can
        # be explored independently from the dashboard sidebar filters.
        ...
        with filter_col1:
            risk_departments = st.multiselect(
                "Department",
                options=risk_department_options,
                default=risk_department_options,
        ...
        with filter_col2:
            risk_years = st.multiselect(
                "Year",
                options=risk_year_options,
                default=risk_year_options,
                key="risk_factor_years",
            )

        risk_filtered = apply_filters(
            data,
            risk_departments,
            risk_years,
            None,
        ...
        if not risk_departments or not risk_years:
            st.info("Choose at least one department and one year to display risk factors.")
        elif risk_filtered.empty:
            st.warning("No records match the selected department and year filters.")
        else:
            risk_rows = []

            for metric, (cutoff, direction) in RISK_RULES.items():
                if metric == "BP":
                    if "SYS" not in risk_filtered.columns or "DIA" not in risk_filtered.columns:
                        continue

                    pct = risk_percent(risk_filtered, metric)
                    sys_cutoff, dia_cutoff = cutoff
                    records_with_data = risk_filtered[["SYS", "DIA"]].dropna().shape[0]
                    count_flagged = (
                        (risk_filtered["SYS"] >= sys_cutoff)
                        & (risk_filtered["DIA"] >= dia_cutoff)
                        & risk_filtered["SYS"].notna()
                        & risk_filtered["DIA"].notna()
                    ).sum()

                    risk_rows.append({
                        "Metric": "Blood Pressure",
                        "Risk Rule": f"SYS ≥ {sys_cutoff} AND DIA ≥ {dia_cutoff}",
                        "Count": int(count_flagged),
                        "Percent Flagged": pct,
                ...
                if direction == "high":
                    count_flagged = (risk_filtered[metric] >= cutoff).sum()
                    rule_text = f"≥ {cutoff}"
                else:
                    count_flagged = (risk_filtered[metric] < cutoff).sum()
                    rule_text = f"< {cutoff}"
                ...
                risk_rows.append({
                    ...
                    "Count": int(count_flagged),
                    "Percent Flagged": pct,
                    "Records with Data": risk_filtered[metric].notna().sum(),
                })
