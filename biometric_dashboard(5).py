...
with tab_departments:
    ...
    if comparison_filtered.empty:
        st.warning("No data available.")
    else:
        comparison_department_options = sorted(
            comparison_filtered["department"].dropna().unique().tolist()
        )

        department_choice = st.multiselect(
            "Departments to show",
            options=["All Departments"] + comparison_department_options,
            default=["All Departments"],
            help=(
                "Choose All Departments to compare every department, or remove "
                "All Departments and choose only the departments you want to display."
            ),
            key="department_comparison_departments",
        )

        if "All Departments" in department_choice:
            comparison_departments = comparison_department_options
        else:
            comparison_departments = [
                department
                for department in department_choice
                if department in comparison_department_options
            ]

        if not comparison_departments:
            st.info(
                "Choose at least one department, or select All Departments, "
                "to display the comparison graph."
            )
        else:
            comparison_data = comparison_filtered[
                comparison_filtered["department"].isin(comparison_departments)
            ].copy()

            summary = summarize_by_department(comparison_data)
            ...
            if summary.empty:
                ...
                st.caption(
                    "Each department is grouped on the x-axis, with separate "
                    "side-by-side bars for each selected year."
                )
...
with tab_risk:
    ...
    if data.empty:
        ...
        risk_department_options = sorted(
            data["department"].dropna().unique().tolist()
        )
        risk_year_options = sorted(
        ...
        with filter_col1:
            risk_department_choice = st.multiselect(
                "Department",
                options=["All Departments"] + risk_department_options,
                default=["All Departments"],
                help=(
                    "Choose All Departments to combine all departments, "
                    "or remove it and choose specific departments."
                ),
                key="risk_factor_departments",
            )

        with filter_col2:
            risk_year_choice = st.multiselect(
                "Year",
                options=["All Years"] + risk_year_options,
                default=["All Years"],
                help=(
                    "Choose All Years to combine every year, "
                    "or remove it and choose specific years."
                ),
                key="risk_factor_years",
            )

        if "All Departments" in risk_department_choice:
            risk_selected_departments = risk_department_options
        else:
            risk_selected_departments = [
                department
                for department in risk_department_choice
                if department in risk_department_options
            ]

        if "All Years" in risk_year_choice:
            risk_selected_years = risk_year_options
        else:
            risk_selected_years = [
                int(year)
                for year in risk_year_choice
                if year != "All Years"
            ]

        risk_filtered = data.copy()

        if risk_selected_departments:
            risk_filtered = risk_filtered[
                risk_filtered["department"].isin(risk_selected_departments)
            ]
        else:
            risk_filtered = risk_filtered.iloc[0:0]

        if risk_selected_years:
            risk_filtered = risk_filtered[
                risk_filtered["year"].astype(int).isin(risk_selected_years)
            ]
        else:
            risk_filtered = risk_filtered.iloc[0:0]

        if risk_filtered.empty:
            st.warning("No records match the selected departments and years.")
        else:
            st.caption(
                f"Showing {len(risk_filtered):,} firefighter record(s) "
                f"from {risk_filtered['department'].nunique()} department(s) and "
                f"{risk_filtered['year'].nunique()} year(s)."
            )
            ...
            for metric, (cutoff, direction) in RISK_RULES.items():
                if metric not in risk_filtered.columns:
                    continue

                metric_data = risk_filtered[metric].dropna()
                records_with_data = len(metric_data)

                if direction == "high":
                    count_flagged = (metric_data >= cutoff).sum()
                else:
                    count_flagged = (metric_data < cutoff).sum()

                pct = risk_percent(risk_filtered, metric)

                risk_rows.append({
                    "Metric": metric,
                    "Risk Rule": f"{'≥' if direction == 'high' else '<'} {cutoff}",
                    "Count Flagged": int(count_flagged),
                    "Percent Flagged": pct,
                    "Records with Data": records_with_data,
                })
            ...
            if not risk_df.empty:
                ...
                st.subheader("Percent of Firefighters Flagged")
                risk_factor_bar_chart(risk_df)

                st.divider()

                st.subheader("Risk Factor Count")
                st.caption(
                    "Number of firefighters who meet each individual risk-factor rule."
                )
                risk_factor_count_chart(risk_df)
            else:
                st.info(
                    "No risk factor information is available for the selected data."
                )
