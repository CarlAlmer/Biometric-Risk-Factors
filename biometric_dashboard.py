...
with tab_risk:
    ...
    if filtered.empty:
        ...
        for metric, (cutoff, direction) in RISK_RULES.items():
            if metric == "BP":
                if "SYS" not in filtered.columns or "DIA" not in filtered.columns:
                    continue

                pct = risk_percent(filtered, metric)
                sys_cutoff, dia_cutoff = cutoff
                records_with_data = filtered[["SYS", "DIA"]].dropna().shape[0]
                count_flagged = (
                    (filtered["SYS"] >= sys_cutoff)
                    & (filtered["DIA"] >= dia_cutoff)
                    & filtered["SYS"].notna()
                    & filtered["DIA"].notna()
                ).sum()
            ...
            if direction == "high":
                count_flagged = (filtered[metric] >= cutoff).sum()
                rule_text = f"≥ {cutoff}"
            else:
                count_flagged = (filtered[metric] < cutoff).sum()
                rule_text = f"< {cutoff}"
